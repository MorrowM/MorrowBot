...
raiseErr :: Monad m => Text -> HandlerT m a
...
handleMessageReactionAdd rct = catchErr $ do
  ...
  forM_ usersToNotify $ \usrId -> do
    ...
    void $
      runDis $
        CreateMessage (channelId dm) $
          T.concat
            [ userName user,
              " reacted to a subscribed message with ",
              emojiName (reactionEmoji rct),
              "!\n> ",
        ...
        runDis $
          CreateMessage (channelId dm) $
            userName (messageAuthor msg) `T.append` " mentioned a watch word in their message:\n> "
              `T.append` clip 300 (messageText msg)
              `T.append` "\n"
    ...
    case messageGuild msg of
      ...
      Just guild -> do
        ...
        case mMsgSubs of
          ...
          msgSubs ->
            send $
              "You are subscribed to the following messages:\n"
                `T.append` T.intercalate ", " (tshow . reactionMessageWatchMessage <$> mMsgSubs)
  Self disable ->
    if disable
...
getText :: WatchWord -> Text
getText (WatchWord _ _ txt) = txt

messageUrl :: Message -> GuildId -> Text
